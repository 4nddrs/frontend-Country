...
const CHART_CONFIG = {
  ...
  activeDotFill: "#f97316",
  activeDotStroke: "#fed7aa",   // orange-200
  tooltipItemColor: "#fb923c",  // orange-400
  tooltipLabel: "Salario",
} as const;
...
function SalaryAreaChart({ data, filterMonth, onFilterChange, formatCurrency }: SalaryAreaChartProps) {
...
function StatusBadge({ state }: { state: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border backdrop-blur-sm ${cfg.bg} ${cfg.border} ${cfg.text} ${cfg.glow}`}
    >
...
const SalaryPayments: React.FC = () => {
  ...
  async function load() {
    try {
      ...
      const normalizedPayments: SalaryPayment[] = dataPayments
        .map((p) => ({ ...p, amount: Number(p.amount), employee: p.employee }))
        .sort((a, b) => b.idSalaryPayment - a.idSalaryPayment);
  ...
  async function maybeRegisterExpense(payment: SalaryPayment) {
    ...
    try {
      const res = await fetch(EXPENSES_URL, {
        ...
        body: JSON.stringify({
          date: today,
          description: `Pago de salario - ${employeeName}`,
  ...
  async function updateItem() {
    ...
    try {
      ...
      setItems((prev) =>
        prev.map((x) => x.idSalaryPayment === normalized.idSalaryPayment ? normalized : x)
      );
  ...
  function onChangeForm<K extends keyof SalaryPaymentCreate>(key: K, value: SalaryPaymentCreate[K]) {
    setForm((prev) => ({ ...prev, [key]: value }));
  }

  function onChangeCreateForm<K extends keyof SalaryPaymentCreate>(key: K, value: SalaryPaymentCreate[K]) {
  ...
  const exportFilteredPDF = async () => {
    try {
      ...
      const body = filtered.map((p, i) => [
        ...
        new Intl.NumberFormat("es-BO", { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(p.amount) + " Bs",
        "",
      ]);
      ...
      autoTable(doc, {
        ...
        foot: [[
          ...
          {
            content: new Intl.NumberFormat("es-BO", { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(total) + " Bs",
  ...
  return (
    <div className="bg-white/0 backdrop-blur-lg p-6 rounded-2xl mb-8 border border-[#167C79] shadow-[0_4px_20px_rgba(0,0,0,0.4)] text-[#F8F4E3]">
      ...
      {/* === Tabla de Pagos === */}
      <AdminSection>
        <h2 className="text-xl font-semibold mb-2 text-teal-400">Pagos Registrados</h2>
            ...
            <tbody>
              {items.map((row) => (
                <tr key={row.idSalaryPayment} className="border-t border-gray-600 text-center align-middle px-6 py-4">
                  ...
                  <td className="p-2">
                    {new Intl.NumberFormat("es-BO", { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(row.amount)} Bs
                  </td>
                  <td className="p-2">
                  ...
                  <td className="p-2 align-middle text-center">
                    <div className="flex items-center justify-center gap-6">
                      <button
                        onClick={() => {
                          setEditingRow(row);
                          setForm({ amount: row.amount, state: row.state, paymentDate: row.paymentDate ?? null, fk_idEmployee: row.fk_idEmployee });
                          setAmountInput(
                            new Intl.NumberFormat("es-BO", { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(row.amount)
                          );
                        }}
      ...
      <AdminSection>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          <div>
            ...
            <select value={filterEmployee} onChange={(e) => setFilterEmployee(Number(e.target.value))} className="w-full">
          ...
          <div>
            <label>Estado</label>
            <select value={filterState} onChange={(e) => setFilterState(e.target.value)} className="w-full">
