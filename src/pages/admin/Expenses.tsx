...
const ExpensesManagement = () => {
  ...
  const fetchExpenses = async () => {
    ...
    try {
      ...
      data.sort((a, b) => (b.idExpenses ?? 0) - (a.idExpenses ?? 0));
      setExpenses(data);
    } catch {
    ...
    try {
      ...
      const res = await fetch(API_URL, {
        ...
        body: JSON.stringify({
          ...
          period: newExpense.period ? `${newExpense.period}-01` : '',
        }),
      });

      if (!res.ok) throw new Error("Error al crear gasto");
      toast.success("Gasto creado correctamente");
      setNewExpense({ date: "", description: "", AmountBsCaptureType: '' as any, period: "" });
      fetchExpenses();
      fetchChartData();
    } catch {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}${id}`, {
        ...
        body: JSON.stringify({
          ...updatedExpense,
          AmountBsCaptureType: cleanedAmount,
          period: updatedExpense.period ? `${updatedExpense.period}-01` : '',
        }),
      });
  ...
  const exportFilteredPDF = async () => {
    try {
      ...
      const titulo = filterMonth
        ? `Egresos del mes ${new Date(filterMonth + '-01').toLocaleDateString("es-ES", { month: "long", year: "numeric" })}`
      ...
      const total = filtered.reduce((acc, e) => acc + Number(e.AmountBsCaptureType || 0), 0);
      ...
      doc.save(`Egresos_${filterMonth || "SinFiltro"}_${now.toISOString().slice(0, 10)}.pdf`);
  ...
  return (
    <div className="bg-white/0 backdrop-blur-lg p-6 rounded-2xl mb-8 border border-[#167C79] shadow-[0_4px_20px_rgba(0,0,0,0.4)] text-[#F8F4E3]">
      ...
      <AdminSection>
        ...
        <div className="flex items-end gap-3 w-full overflow-x-auto py-5">
          ...
          <div className="flex-1 min-w-0">
            ...
            <input
              ...
              onFocus={() => {
                if (String(newExpense.AmountBsCaptureType) === '0') {
                  setNewExpense((prev) => ({ ...prev, AmountBsCaptureType: '' as any }));
                }
              }}
              onChange={(e) =>
        ...
        {/* === Gráfico de gastos — estilo Sales Overview, color morado === */}
        <AnimatedBackground className="mt-6 rounded-xl border border-purple-900/30">
          {/* Header: título izq, filtro der */}
          ...
          <ResponsiveContainer width="100%" height={240}>
            <AreaChart data={chartData} margin={{ top: 10, right: 20, left: 10, bottom: 0 }}>
              ...
              <Area
                ...
                dot={false}
                activeDot={{ r: 5, fill: '#a855f7', stroke: '#d8b4fe', strokeWidth: 2 }}
              />
            </AreaChart>
          </ResponsiveContainer>
        </AnimatedBackground>
            ...
            {expenses.map((expense) => (
              ...
              >
                <div className="flex flex-col items-center gap-2 py-5">
                  ...
                  <span className="text-xs uppercase tracking-[0.3em] text-slate-400">Gasto</span>
                </div>
                ...
                <div className="px-6 pb-6 space-y-4 text-sm text-slate-200">
                  ...
                  <div className="space-y-2 text-center">
                    ...
                    <p><span className="font-medium text-slate-400">Monto:</span> <span className="text-red-300">Bs {expense.AmountBsCaptureType}</span></p>
                    <p><span className="font-medium text-slate-400">Periodo:</span> {expense.period?.slice(0, 7)}</p>
                  </div>
                    ...
                    <button
                      onClick={() => {
                        ...
                        setEditingExpense({
                          ...
                          period: expense.period?.slice(0, 7) ?? '',
                        });
                      }}
